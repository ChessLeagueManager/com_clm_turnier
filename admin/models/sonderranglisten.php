...
class CLM_TurnierModelSonderranglisten extends JModelList {
     ...
     * <ul>
     * <li>Saison: nicht archiviert, veröffentlicht</li>
    ...
    protected function _getQuery() {
    	...
    	$query->select($this->_db->quoteName(explode(',', 'r.id,r.name,r.ordering,s.name,t.name'), 
    			explode(',', 'id,name,ordering,saison,turnier')));
    	...
    	$query->join('', $this->_db->quoteName('#__clm_turniere_sonderranglisten', 'r') . ' ON ' .
    			$this->_db->quoteName('r.turnier') . ' = ' .
    ...
    protected function getListQuery() {
        ...
        if (! empty($search)) {
            if (stripos($search, 'id:') === 0) {
                ...
                $query->where($this->_db->quoteName('r.name') . ' LIKE ' . $search);
                $query->orWhere($this->_db->quoteName('t.name') . ' LIKE ' . $search);
            }
        }
