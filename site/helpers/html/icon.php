...
abstract class JHtmlIcon {
	...
	public static function print_popup($state, $params, $attribs = array(), $legacy = false) {
		...
		$text = JLayoutHelper::render('joomla.content.icons.print_popup', array (
	...
	public static function print_screen($state, $params, $attribs = array(), $legacy = false) {
		$text = JLayoutHelper::render('joomla.content.icons.print_screen', array (
				'params' => $params, 'legacy' => $legacy ));
	...
	public static function email($state, $params, $attribs = array(), $legacy = false) {
		...
		$url = 'index.php?option=com_mailto&tmpl=component&template=' . $template .
				'&link=' . MailtoHelper::addLink($link);

		$status = 'width=400,height=450,menubar=yes,resizable=yes';

		$text = JLayoutHelper::render('joomla.content.icons.email', array (
				'params' => $params, 'legacy' => $legacy ));
	...
	public static function filter($state, $params, $attribs = array(), $legacy = false) {
		...
		$text = JLayoutHelper::render('icons.filter', array (
				'params' => $params, 'attribs' => $attribs, 'legacy' => $legacy ), JPATH_CLM_TURNIER_COMPONENT);
